...
L'helper di osservazione è un normalizzatore di evidenza, non un adattatore del controllore laser: non apre alcun collegamento di rete o seriale, non modifica impostazioni, non carica lavori, non arma né spara. [Laser Controller Evidence Boundary](docs/CONTROLLER_EVIDENCE_BOUNDARY.md) definisce la validazione richiesta prima di una reale integrazione.
...
│   ├── gpio_mqtt_emulator.py    # Emulatore GPIO/MQTT fedele al protocollo (test double realistico)
│   └── test_gpio_mqtt_emulator.py # Comportamento del bridge rispetto all'emulatore GPIO/MQTT
├── tools/
...
**Reale oggi:** versione `0.1.0`, un nucleo di pianificazione fail-safe testato in locale (`LaserSafetySnapshot` + `LaserCellBridge`) appoggiato sulla porta di lavoro condivisa di `HYDRA-UMC-SDK`, normalizzazione rigorosa e in sola lettura dell'evidenza di sicurezza che distingue un lavoro realmente in pausa (`HOLDING`) da uno realmente in taglio (`RUNNING`), una lettura GPIO reale e neutrale rispetto al controllore (`GpioSafetyProbe`) per le 3 protezioni indipendenti chiave/involucro/interblocco, un vero trasporto MQTT di stato/evidenza (`mqtt_transport.py`), una suite `unittest` deterministica di quarantanove test, e script build-test non mutanti collegati alla CI con checkout dell'SDK.
