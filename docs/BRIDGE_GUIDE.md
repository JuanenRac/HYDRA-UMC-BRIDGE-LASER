...
`gpio_safety.py`'s `GpioSafetyProbe.read_snapshot()` is this bridge's first real transport: it reads the same 3 independent safeguards (key/enclosure/interlock) from real GPIO lines via libgpiod v2 (`gpiod`, optional `[gpio]` extra, imported lazily inside `open_gpio_safety_lines()`) instead of a saved mapping. It is deliberately controller-neutral - a key switch, door sensor or interlock relay's own feedback contact is universal wiring across laser cutters regardless of brand, so reading it directly over GPIO needs no controller-specific protocol decision. A GPIO read failure fails all 3 safeguards closed, mirroring `observation.py`'s own "missing/invalid means unsafe" rule. The reading logic is written against a small `GpioLineReader` interface so it is unit-testable with an in-memory fake, with no real GPIO chip required. `open_gpio_edge_watcher()`/`watch_for_interlock_edges()` add real-time monitoring on top of that same on-demand read path - they block on libgpiod v2's own kernel edge-event notifications (`Edge.BOTH`) instead of only re-reading levels when an unrelated inbound message happens to ask.

`mqtt_transport.py` reaches this same real logic over `HYDRA-UMC-MQTT-BROKER`, this bridge's second real transport. Unlike the sibling CNC/PRINTER3D bridges it exposes no real actuation command here either - `LaserCellBridge` cannot arm or fire a laser, so `LaserMqttBridge.handle_message()` only routes `hydra/bridges/laser/cmd/{status,job}`: `status` re-reads the 3 GPIO safeguards and publishes `hydra/bridges/laser/state` (retained); `job` accepts a `BridgeJob` in the shared SDK's `job_to_dict()` wire shape and answers with a `GateDecision` on `.../cmd/job/result`. Dispatch is pure enough to unit-test with the same in-memory fake GPIO lines, no real broker or GPIO chip required - `paho-mqtt` (optional `[mqtt]` extra) is only imported, lazily, inside `run_forever()`. `run_forever()`'s optional `gpio_chip_path`/`key_line_offset`/`enclosure_line_offset`/`interlock_line_offset` arguments start a background daemon thread on the real edge-watcher above, publishing an updated, retained `state` the instant a safeguard line transitions instead of only on the next unrelated `cmd/status`/`cmd/job` poll; omitting them keeps the previous on-demand-only behavior.
